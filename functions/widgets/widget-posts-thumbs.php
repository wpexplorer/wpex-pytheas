...
class wpex_posts_thumb_widget extends WP_Widget {
    ...
    public function wpex_posts_thumb_widget() {
       parent::__construct(
            'wpex_posts_thumb_widget',
            esc_html__( 'Standard Posts w/ Thumbs', 'pytheas' )
        );
    }
    ...
    public function form( $instance ) {	
      extract( wp_parse_args( (array) $instance, array(
            'title'    => __( 'Featured Posts', 'pytheas' ),
			'category' => '',
			'number'   => '5',
			'offset'   => '0'
		) ) ); ?>
        ...
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'category' ) ); ?>"><?php _e( 'Select a Category:', 'pytheas' ); ?></label>
            <br />
            <select class='att-select' name="<?php echo esc_attr( $this->get_field_name( 'category' ) ); ?>" id="<?php echo esc_attr( $this->get_field_id( 'category' ) ); ?>">
		...
		<p>
          <label for="<?php echo esc_attr( $this->get_field_id( 'number' ) ); ?>"><?php _e( 'Number to Show:', 'pytheas' ); ?></label> 
          <input class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'number' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'number' ) ); ?>" type="text" value="<?php echo esc_attr( $number ); ?>" />
        </p>
		<p>
          <label for="<?php echo esc_attr( $this->get_field_id( 'number' ) ); ?>"><?php _e( 'Offset (the number of posts to skip):', 'pytheas' ); ?></label> 
